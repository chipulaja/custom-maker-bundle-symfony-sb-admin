...
use Symfony\Bundle\MakerBundle\Renderer\FormFilterTypeRenderer;
...
final class MakeCrud extends AbstractMaker
{
    ...
    public function __construct(
        ...
        private FormFilterTypeRenderer $formFilterTypeRenderer
    ) {
    ...
    public function generate(InputInterface $input, ConsoleStyle $io, Generator $generator): void
    {
        ...
        $readerClassDetails = $generator->createClassNameDetails(
        ...
        do {
            $formFilterClassDetails = $generator->createClassNameDetails(
                $entityClassDetails->getRelativeNameWithoutSuffix().($filterIter ?: '').'Type',
                'Form\\',
                'FilterType'
            );
            ++$filterIter;
        ...
        $readerVar = lcfirst($this->inflector->singularize($readerClassDetails->getShortName()));
        ...
        $generator->generateReader(
            ...
            if (preg_match('/be generated because it already exists/i', $e->getMessage())) {
                // do nothing
            } else {
                throw new RuntimeCommandException($e->getMessage());
            }
        }
        ...
        $generator->generateController(
            ...
            array_merge([
                    ...
                    // @legacy - Remove when support for Symfony <6 is dropped
                    'use_render_form' => Kernel::VERSION_ID < 60200,
                    'reader_class_name' => $readerClassDetails->getShortName(),
                    'reader_var' => $readerVar,
                ],
                $repositoryVars
            )
        );
        ...
        $templates = [
            '_form' => [
                'route_name' => $routeName,
                'js_path' => "js/",
                'fields' => $entityDoctrineDetails->getFormFields(),
            ],
            'edit' => [
                'entity_class_name' => $entityClassDetails->getShortName(),
                'entity_twig_var_singular' => $entityTwigVarSingular,
